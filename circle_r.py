...
for app in apps:
    if app in OLD_DATA:
        write(f"{app}")
        rprint(f"[bold white]App:[/] {app}")
    else:
        write(f"{app} - NEW!!")
        rprint(f"[bold cyan]New:[/] {app}")
        new_apps += 1
    #print(app.text)
rprint("\n[bold green]+ SUMMARY[/]")
green_pipe = "[bold green]|[/]"
rprint(f"""{green_pipe} [bold white]Amount of apps before:[/] {before}
{green_pipe} [bold white] Amount of apps found:[/] {now} 
{green_pipe} [bold white] New apps found:[/] {new_apps}""")
